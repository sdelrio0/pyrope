...
import { isNumber, isArray, isBoolean, isString, pluck, isObject, isFunction, isEmpty } from 'underscore';
...
export const update = (opts) => new Promise((resolve, reject) => {
  ...
  let { tableName, index, fields, beforeHook } = opts;
  ...
  if(fields === undefined || !isObject(fields)) return reject(`update() > 'fields' is undefined or not an object.`);
  ...
  findByIndex({
    ...
    .then(item => {
      if(isEmpty(fields)) {
        ...
        return _update({...opts, index: {uuid: item[0].uuid, createdAt: item[0].createdAt}})
