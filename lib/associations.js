...
export const dissociate = ( opts ) => {
  ...
  return findByIndex({
    ...
    if(otherIndexValues === null){
      ...
      deleteRequests.map(currItem => ({DeleteRequest: {Key: {uuid: currItem.uuid, createdAt: currItem.createdAt}}}));
    }
    
    if(notFound) {
      return Promise.reject(`${otherIndexKey}[${notFound}] is not associated to ${indexKey}[${indexValues}]`);
    }
