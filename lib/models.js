...
import { filter, isArray, isObject, sortBy, find, isString, mapObject, omit } from 'underscore';
...
export default class PyropeModel {
  ...
  constructor(schema, opts = {}) {
    ...
    this.defaultQueryKey = 'uuid';  // opts.defaultQueryKey || 'uuid';
  ...
  get(index) {
    return new Promise((resolve, reject) => {
      ...
      const indexValue = index[Object.keys(index)[0]];
      ...
      pyrope.findByIndex({
        ...
        .then(records => {
          if(records === false) {
            reject(`${this.humanName} '${indexValue}' not found.`);
  ...
  /**
   * !! Does not check if the record with a given key exists.
   * @param fields
   * @param opts
   */
  create(fields = {}, opts = {}) {
  ...
  update(index, fields = {}, opts = {}) {
    return new Promise((resolve, reject) => {
      ...
      const preAssociationsHook = (fields) => new Promise((res, rej) => {
        ...
        res(omit(fields, fieldsToRemove));
      });
  ...
  /**
   *
   * @todo: refactor updateAssociationHook with batchWrite
   * @param index
   * @param opts
   * @returns {Promise}
   */
  destroy(index, opts = {}) {
    return new Promise((resolve, reject) => {
      ...
      const updateAssociationsHook = (fields) => new Promise((resolve, reject) => {
        ...
        let nullifyAssocKeys = (filter(this.fields, f => f.dependent === 'nullify' || f.dependent === 'destroy'));
          ...
          destroyAssocKeys.reduce((prevPromise, currKey, index, arr) =>
            prevPromise.then(() => {
              ...
              return pyrope.getAssociations({
                tableName: this.buildAssociationTableName(currKey),
                items: [
  ...
  buildAssociationTableName(associationKey) {
    let table = sortBy([this.name, associationKey].map(t => Inflector.tableize(t)), t => t).join('_');
