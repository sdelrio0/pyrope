...
export default class PyropeModel {
  ...
  constructor(schema, opts = {}) {
    ...
    this.tableName = this.tablePrefix +
      (process.env.NODE_ENV === 'test' ? '_test_' : '') + (opts.tableName || Inflector.tableize(schema.name))
      + this.tableSuffix;
  }
          ...
          destroyAssocKeys.reduce((prevPromise, currKey, index, arr) =>
            prevPromise.then(() => {
              ...
              return pyrope.getAssociations({
                tableName: buildAssociationTableName(this.name, currKey, this.tablePrefix, this.tableSuffix),
                items: [
  ...
  getChild(uuid, childIndexKey, childResolver, opts = {}) {
    return new Promise((resolve, reject) => {
      ...
      const tableName = opts.table || buildAssociationTableName(this.name, childIndexKey, this.tablePrefix, this.tableSuffix);
